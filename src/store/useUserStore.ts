...
export const useUserStore = create<UserState>(set => ({
    ...
    setLoadingProfilePic: (isLoadingProfilePic: boolean) => set({ isLoadingProfilePic }),
    setUser: (user: UserState["user"]) => set({ user }),
    setProfilePic: (profilePic: string) => set((state)=> ({ user: state.user ? { ...state.user, image: profilePic } : null})),
}));
